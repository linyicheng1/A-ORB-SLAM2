...
void LocalMapping::CreateNewMapPoints()
{
    ...
    int nnew=0;
    int mp_num = 0;
    for (const auto& mp:mpCurrentKeyFrame->GetMapPointMatches())
        if (mp != NULL) mp_num ++;
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        matcher.SearchForTriangulationWithGMS(mpCurrentKeyFrame,pKF2,F12,vMatchedIndices,false);
//        if (mpCurrentKeyFrame->mnFrameId > 150)
//        {
//            cv::Mat show, img1, img2;
//            img1 = mpCurrentKeyFrame->pyr_[0].clone();
//            img2 = pKF2->pyr_[0].clone();
...
//                cv::DMatch m;
//                m.queryIdx = ikp;m.trainIdx = ikp;
//                matches.emplace_back(m);
//                pts1.emplace_back(kp1);
//                pts2.emplace_back(kp2);
//            }
//            for (int j = 0; j < mpCurrentKeyFrame->mvKeys.size(); j++)
//            {
//                if (mpCurrentKeyFrame->GetMapPoint(j) == nullptr)
//                {
//                    cv::circle(img1, mpCurrentKeyFrame->mvKeys.at(j).pt, 3, cv::Scalar(255, 0, 0), 3);
//                }
//            }
//            for (int j = 0; j < pKF2->mvKeys.size(); j++)
//            {
//                if (pKF2->GetMapPoint(j) == nullptr)
//                {
//                    cv::circle(img2, pKF2->mvKeys.at(j).pt, 3, cv::Scalar(255, 0, 0), 3);
//                }
//            }
//            cv::drawMatches(img1, pts1, img2, pts2, matches, show);
//            cv::imshow("show", show);
//            cv::waitKey(1);
//        }
        ...
        //std::cout<<" map match num: "<<nmatches<<" current has "<<mp_num;
        int last = nnew;
...
//        for (int id = 0;id < mpCurrentKeyFrame->GetMapPointMatches().size(); id ++){
//            const auto& mp = mpCurrentKeyFrame->GetMapPoint(id);
//            if (mp != NULL){
//                const auto& kp = mpCurrentKeyFrame->mvKeys.at(id);
//                cv::circle(show, kp.pt, 5, cv::Scalar(random()*255,random()*255,random()*255));
//                std::cout<<"pt : x "<<kp.pt.x<<" y "<<kp.pt.y<<std::endl;
//                cv::imshow("show", show);
//                cv::waitKey(2);
//            }
//        }
//    }
//    if (nnew < 50 && mp_num < 200)
//    {
//        cv::Mat show1 = mpCurrentKeyFrame->pyr_[0].clone();
//        cv::Mat show2 = vpNeighKFs[0]->pyr_[0].clone();
//        cv::cvtColor(show1, show1, CV_GRAY2BGR);
//        cv::cvtColor(show2, show2, CV_GRAY2BGR);
//        for (int j = 0;j < mpCurrentKeyFrame->mvKeys.size();j ++)
//        {
//            const auto& kp = mpCurrentKeyFrame->mvKeys.at(j);
//            if (mpCurrentKeyFrame->GetMapPoint(j) == NULL)
//            {
//                cv::circle(show1, kp.pt, 2, cv::Scalar(0,255,0));
//            }
//            else
//            {
//                cv::circle(show1, kp.pt, 2, cv::Scalar(0,0,255));
//            }
//        }
//        for (int j = 0;j < vpNeighKFs[0]->mvKeys.size();j ++)
//        {
//            const auto& kp = vpNeighKFs[0]->mvKeys.at(j);
//            if (vpNeighKFs[0]->GetMapPoint(j) == NULL)
//            {
//                cv::circle(show2, kp.pt, 2, cv::Scalar(0,255,0));
//            }
//            else
//            {
//                cv::circle(show2, kp.pt, 2, cv::Scalar(0,0,255));
//            }
//        }
//        cv::imshow("current: ", show1);
//        cv::imshow("pKF2: ", show2);
//        cv::waitKey(0);
//    }
    //std::cout<<" add new points: "<<nnew<<std::endl;
}
