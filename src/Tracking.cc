...
bool Tracking::TrackWithMotionModel()
{
    ...
    if(mSensor!=System::STEREO)
        ...
        th=7;
    // TODO match by optical flow
    int nmatches = matcher.SearchByProjectionWithOF(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR);
...
{
    //std::cout<<" Create New Key Frame "<<std::endl;
...
//    std::cout<<" current id: "<<mCurrentFrame.mnId<<"ref "<<mpReferenceKF->mnFrameId<<std::endl;
    ...
    for (auto &kf:kfs)
    {
        int num = matcher.SearchByProjectionWithOF(mCurrentFrame, kf, 1, true);
//        {// 可视化
//            if (mCurrentFrame.mnId > 1550)
//            {
//                cv::Mat show1 = mCurrentFrame.pyr_[0].clone();
//                cv::Mat show2 = kf->pyr_[0].clone();
//
...
//    {
//        const auto &kf = mvpLocalKeyFrames.at(mvpLocalKeyFrames.size()-i-1);
//        int num = matcher.SearchByProjectionWithOF(mCurrentFrame, kf, 1, true);
//        std::cout<<"kf id "<<kf->mnFrameId<<" match local "<<num<<std::endl;
//        cnt ++;
//        match_num += num;
//        if (cnt >= 10)
//            break;
//    }
//    int num = matcher.SearchByProjectionWithOF(mCurrentFrame, mpReferenceKF, 1, true);
//    match_num += num;
//    if (match_num + track_num < 100)
//    {
//        cnt = 0;
//        auto kf = local_queue.back();
//        cv::Mat show1 = mCurrentFrame.pyr_[0].clone();
//        cv::Mat show2 = kf->pyr_[0].clone();
//        cv::cvtColor(show1, show1, CV_GRAY2BGR);
//        cv::cvtColor(show2, show2, CV_GRAY2BGR);
//        for (int i = 0;i < kf->mvKeys.size(); i++){
//            if (kf->GetMapPoint(i) != nullptr){
//                cv::circle(show2, kf->mvKeys.at(i).pt, 2, cv::Scalar(255, 0, 0));
//            }
//            else{
//                cv::circle(show2, kf->mvKeys.at(i).pt, 2, cv::Scalar(0, 255, 0));
//            }
//        }
//        for (auto & mvKey : mCurrentFrame.mvKeys){
//            cv::circle(show1, mvKey.pt, 2, cv::Scalar(0, 255, 0));
//        }
