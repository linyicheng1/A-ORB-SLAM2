...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, bool extract)
    :mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    frameSize = imGray.size();
...
void Frame::add_pts(const vector<cv::KeyPoint> &pts, const vector<MapPoint *> &mps)
{
    // 添加通过光流法跟踪的特征点
//    mvKeys.clear();
...
//        cv::Mat show = cv::Mat(image.rows, image.cols, CV_8UC1, cv::Scalar(255));
//        for (int i = 0;i < add_pts.size();i ++)
//        {
//            if (index[i] != -1)
//            {
//                cv::circle(show, add_pts.at(i).pt, 3, 0, -1);
//            }
//        }
//        cv::imshow("mask", mask);
//        cv::imshow("show", show);
//        cv::waitKey(0);
//        std::cout<<"new add pts "<<pt_num<<std::endl;
        // 将新提取的特征添加
        for (int i = 0;i < add_pts.size();i ++)
        {
            if (index[i] != -1)
